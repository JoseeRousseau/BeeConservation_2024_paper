...
write.csv(allBees, here("Data", "BeeData_Conservation_paper", "bees_Chesshire2021_USA_20231011.csv"), row.names = FALSE) # 1,991,840
...
### Import if needed
allBees <- read.csv(here("Data", "BeeData_Conservation_paper", "bees_Chesshire2021_USA_20231011.csv"))
...
### Extract total number of records and associated percent of total records for each institution
num.perInst <- allBees %>%
  group_by(institutionCode) %>%
  summarise(n = n(), .groups = "drop") %>%
  mutate(percent = n/(nrow(allBees)))

### Check records for institutions with at least 5% of all records
AMNH <- allBees[which(allBees$institutionCode == "AMNH"),]
BISON <- allBees[which(allBees$institutionCode == "BISON"),]
iNaturalist <- allBees[which(allBees$institutionCode == "iNaturalist"),]
KU <- allBees[which(allBees$institutionCode == "KU"),]
USDA.ARS <- allBees[which(allBees$institutionCode == "USDA-ARS"),]
